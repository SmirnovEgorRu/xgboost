...
void QuantileHistMaker::Builder::BuildLocalHistograms(
  ...
  printf("BUILD: %8zu | %10.5f %10.5f %10.5f | %10.5f %10.5f %10.5f\n", nodes_for_explicit_hist_build_.size(), t2-t1, t3-t2, t4-t3, tt1, tt2, tt3);
...
void QuantileHistMaker::Builder::ExpandWithDepthWise(
  ...
  qexpand_depth_wise_.emplace_back(ExpandEntry(0, -1, p_tree->GetDepth(0), 0.0, timestamp++));
  ++num_leaves;
...
void QuantileHistMaker::Builder::ExpandWithLossGuide(
  ...
  ExpandEntry node(0, -1, p_tree->GetDepth(0), snode_[0].best.loss_chg, timestamp++);
